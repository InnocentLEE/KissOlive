/**
 * 利用label辅助点击file框 可完美解决在ie中的“访问限制”问题（即必须是用户点击才能进行文件上传）
 * 仅对class为custom-file-input应用
 */
$.customFileInput = function() {
	$('input[type=file].custom-file-input').each(
			function(i, elem) {
				var $input = $(this);
				...
				var $btn = $('<a class="btn"><label for="' + $input.prop("id")
						+ '">' + btnTitle + '</label></a>');
				$input.before($btn);
				...
				$input.change(function() {
					...
					$btn.after('<span class="name">' + $(this).val()
							+ '</span>');
				});
